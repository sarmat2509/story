...
export const DIRECTOR_CACHE_KEY = 'director_rules_v32_explicit_character_position';
...
- Each cameraComposition.characters row must include characterRef, name, position, description, and outfitId. setting describes scene objects/state only; shot describes only the camera and framing. Never use setting or shot to introduce or collectively stage "the characters" or a group.
...
- FIELD SEPARATION: setting contains only visible object/environment changes, never a collective phrase such as "the characters gather". cameraComposition.shot contains only shot size, camera position, viewing direction, eye level, and framing; never "a shot of the characters" or a collective character action. Put each visible character exactly once in cameraComposition.characters with its own concrete position relative to the frame and the main scene object.
...
For these user-selected characters, keep characters[].description minimal and reference-compatible. Use it only for a short neutral anchor when required by the schema. In sceneVisual.cameraComposition.characters[].position, give the concrete frame zone and relation to the main scene object. In sceneVisual.cameraComposition.characters[].description, describe only pose, expression, gaze, head turn, hand use, action, and temporary visibility/occlusion; do not repeat position there. Use literal observable language only; never use a simile, metaphor, species comparison, or transformation wording to describe movement. Spell out the real human pose instead (for example, "swimming horizontally in the current, arms extended" rather than "swimming like a mermaid"). Do NOT restate, paraphrase, or sneak in stable identity traits there such as hairstyle, ponytail/braid details, hair color, eye color, freckles, face shape, skin tone, body build, age markers, or other enduring appearance details. If you catch yourself naming a permanent face/hair feature from the sheet, remove it and replace it with a neutral visible action description. Put wardrobe only in outfits[].description.`
    : ''
}
...
Each cameraComposition.characters[] row MUST include: characterRef, name, position, description, outfitId (exact outfits[].id for that character in this shot). position states frame zone plus relation to the main scene object; description states pose/action/expression without repeating position. Each outfits[] row must carry the same characterRef as its character. Non-empty characters array; outfitId is a technical binding enforced like environmentId. Detailed wardrobe descriptions are only for child/person/human characters; animals, creatures, objects, vehicles, and environmental beings use "natural appearance".
