...
function selectTemplate(
  ...
  const ageTemplatePool = options.templates
    ? templatePool.filter((template) =>
        template.allowedAgeGroups.includes((ageGroup || '4-5') as GraphicNovelAgeGroup)
      )
  ...
  const fallbackCandidates = templatePool.filter((template) => template.panelCount === requiredPanelCount);
  ...
  return shuffleEqualScoreGroups(scored, randomSource).map((item) => item.template);
}
...
export function planGraphicNovelLayouts(params: {
  ...
  return params.pages.map((page) => {
    ...
    if (params.preferredTemplateId) {
      const preferred = templatePool.find((template) => template.id === params.preferredTemplateId);
      if (!preferred) {
    ...
    if (candidates.length === 0) {
      throw new Error(
        `No predefined graphic novel template for page ${page.pageNumber} with ${panels.length} panels`
      );
    }
    ...
    const fallbackCandidates = selected
      ...
      : plannedCandidates.filter((candidate) =>
          candidate.fit.overflowCount ===
          Math.min(...plannedCandidates.map((item) => item.fit.overflowCount))
        );
    ...
    const plannedPanels =
      finalSelected?.plannedPanels ?? planPanelsForTemplate(panels, template, params.bubbleTextSizing);
