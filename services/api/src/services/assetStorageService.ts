...
export class AssetStorageService {
  ...
  /**
   * Save outfit plate cache image (garment reference, global reuse)
   * Path: outfit_plate_cache/{cacheId}.png
   */
  async saveOutfitPlateCacheImage(
    cacheId: string,
    buffer: Buffer,
    ...
    const ext = mimeType.includes('jpeg') || mimeType.includes('jpg') ? '.jpg' : '.png';
    const storagePath = `outfit_plate_cache/${cacheId}${ext}`;

    if (this.provider === 'local') {
      const fullPath = path.join(this.localUploadDir, storagePath);
      await fs.mkdir(path.dirname(fullPath), { recursive: true });
      await fs.writeFile(fullPath, buffer);
      logger.info({ cacheId, storagePath, size: buffer.length }, 'Outfit plate cache image saved');
      return {
        storagePath,
        storageUrl: `/api/v1/assets/${storagePath}`,
      };
      ...
      const storageUrl = config.storage.cdnUrl
        ? `${config.storage.cdnUrl}/${storagePath}`
        : `/api/v1/assets/${storagePath}`;
      logger.info({ cacheId, storagePath }, 'Outfit plate cache image saved to S3');
      return { storagePath, storageUrl };
    }
  }
