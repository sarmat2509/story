...
>
> Актуальность: 18 июля 2026 года. Все дети, взрослые, животные и фантастические герои в документе вымышлены и создаются без фотографий.
...
Golden допускает 20 историй и 5 комиксов за расчётный период. Для 21 истории, включая 7 комиксов, потребуется:

1. либо один демонстрационный аккаунт в течение двух расчётных периодов;
2. либо специальный лимит для демонстрационного аккаунта.

Предпочтителен один аккаунт: так профили детей и персонажей сохраняются без дублирования и легче поддерживать визуальную консистентность.
...
Перед генерацией и публикацией каталога необходимо:

- добавить `storyFormat` в публичный контракт; предполагаемые значения: `story`, `comic`, `mixed` — окончательные имена должны совпадать с кодовой схемой;
- для комикса публично отдавать страницы, изображения страниц и текстовые пузыри;
- для mixed-истории публично отдавать порядок чтения прозы и комикс-блоков;
- научить публичный reader отображать `comic` и `mixed`;
- добавить на карточки каталога значки «История», «Комикс» и «История + комикс»;
- не добавлять отметку «3 персонализированные иллюстрации».
- передавать в prompt генерации детского turnaround текущий возраст, вычисленный сервером из `birthDate`; возраст не должен дублироваться в `aiGeneratedDescription`.

Статус этого раздела: передача текущего возраста в детский turnaround реализована и проверена; остальные требования необходимо проверять отдельно в коде.
...
| Polski | „Maja i kompas dżungli” | 5 | Приключение в джунглях | Майя, Пико, Орби | Фетровая аппликация |
...
| Polski | „Dwa domy, jedna opowieść” | 3 | Семьи и культуры | Лина, Момо, Кико | Цветные карандаши |

Перед генерацией каждый синопсис нужно проверить на соответствие названию после замены состава. Например, бабушка в «La lanterne de Mamie» может оставаться сюжетным неперсонализированным персонажем, но основным визуально консистентным составом остаются Майя и Орби.
...
- **18.07.2026:** исходная отметка «генерация ещё не начата» закрыта: семь детских профилей и их turnaround-листы созданы в production. Истории и 15 дополнительных персонажей ещё не генерировались.
...
- До контрольных историй решить, принимаем ли Майю как есть и регенерируем ли Рави и Зури. Существующие изображения не перегенерировались автоматически после визуального просмотра.

## 12. Открытые проверки

- Сверить названия 17 тем с фактическими идентификаторами scenario cards.
- Проверить точные enum-значения `storyFormat` в коде до изменения публичного контракта.
- Проверить, входят ли 15 генераций персонажей в один период Golden именно как 15 уникальных model sheets.
