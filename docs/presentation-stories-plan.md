...
- Исправить несовпадение ключа magical shōjo: UI/shared передают `retro_magical_shojo`, а prompt-движок содержит `retro_shojo_fantasy`; без исправления выбранный стиль откатывается к `soft_watercolor`.
- Разделить производство на два расчётных периода либо выдать демонстрационному аккаунту специальный лимит: текущие 20 историй и 5 комиксов не покрывают план из 21 истории и 7 комиксов.
- После исправления стиля сгенерировать три контрольных материала — обычную историю, комикс и mixed — затем проверить публикацию, пузырьки и mixed reading order на фактических production-данных.
