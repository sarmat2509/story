...
assert.deepEqual(
  [...PUBLIC_SEO_LOCALES],
  ['en', 'uk'],
  'public SEO locales should cover indexed English and Ukrainian public pages'
);
...
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/support<\/loc>/);
...
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/uk\/<\/loc>/);
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/uk\/pricing<\/loc>/);
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/uk\/stories<\/loc>/);
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/uk\/blog<\/loc>/);
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/uk\/support<\/loc>/);
assert.match(emptySitemap, /<loc>https:\/\/wondertales\.art\/uk\/blog\/adhd-story-attention<\/loc>/);
assert.doesNotMatch(emptySitemap, /<loc>https:\/\/wondertales\.art\/(?:ru|es|de|fr|pl)\//);
...
assert.equal(buildPublicLegalPath('terms', 'ru'), '/terms');
assert.equal(buildPublicLegalPath('terms', 'es'), '/terms');
assert.equal(buildPublicLegalPath('privacy', 'en'), '/privacy');
assert.equal(buildPublicLegalPath('privacy', 'uk'), '/uk/privacy');
assert.equal(buildPublicLegalPath('privacy', 'de'), '/privacy');
assert.equal(buildPublicLegalPath('privacy', 'pl'), '/privacy');
...
for (const { name, value } of nginxConfigs) {
  ...
  assert.match(value, /location ~ \^\/\(uk\)\/terms\/\?\$\s*\{[\s\S]*?\/ssr\/legal\/terms\/\$1/, `${name} should route Ukrainian terms to API SSR`);
  assert.match(value, /location ~ \^\/\(uk\)\/privacy\/\?\$\s*\{[\s\S]*?\/ssr\/legal\/privacy\/\$1/, `${name} should route Ukrainian privacy to API SSR`);
assert.match(value, /location = \/stories\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/stories/, `${name} should route exact /stories to API SSR`);
  assert.match(value, /location ~ \^\/\(uk\)\/stories\/\?\$\s*\{[\s\S]*?\/ssr\/stories\/catalog\/\$1/, `${name} should route Ukrainian stories route to API SSR`);
  assert.match(value, /location ~ \^\/\(uk\)\/support\/\?\$\s*\{[\s\S]*?\/ssr\/support\/\$1/, `${name} should route Ukrainian support route to API SSR`);
  assert.match(value, /location = \/blog\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/blog/, `${name} should route blog index to API SSR`);
  assert.match(value, /location ~ \^\/\(uk\)\/blog\/\?\$\s*\{[\s\S]*?\/ssr\/blog\/index\/\$1/, `${name} should route Ukrainian blog index to API SSR`);
  assert.match(value, /location \^~ \/blog\/\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/blog\//, `${name} should route blog articles to API SSR`);
  assert.match(value, /location ~ \^\/\(uk\)\/blog\/\(\[\^\/\?\]\+\)\/\?\$\s*\{[\s\S]*?\/ssr\/blog\/\$1\/\$2/, `${name} should route Ukrainian blog articles to API SSR`);
...
assert.match(commonSsrRoutes, /location ~ \^\/\(uk\)\/terms\/\?\$\s*\{[\s\S]*?\/ssr\/legal\/terms\/\$1/);
assert.match(commonSsrRoutes, /location ~ \^\/\(uk\)\/privacy\/\?\$\s*\{[\s\S]*?\/ssr\/legal\/privacy\/\$1/);
assert.match(commonSsrRoutes, /location = \/stories\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/stories/);
assert.match(commonSsrRoutes, /location ~ \^\/\(uk\)\/stories\/\?\$\s*\{[\s\S]*?\/ssr\/stories\/catalog\/\$1/);
assert.match(commonSsrRoutes, /location ~ \^\/\(uk\)\/support\/\?\$\s*\{[\s\S]*?\/ssr\/support\/\$1/);
assert.match(commonSsrRoutes, /location = \/blog\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/blog/);
assert.match(commonSsrRoutes, /location ~ \^\/\(uk\)\/blog\/\?\$\s*\{[\s\S]*?\/ssr\/blog\/index\/\$1/);
assert.match(commonSsrRoutes, /location \^~ \/blog\/\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/blog\//);
assert.match(commonSsrRoutes, /location ~ \^\/\(uk\)\/blog\/\(\[\^\/\?\]\+\)\/\?\$\s*\{[\s\S]*?\/ssr\/blog\/\$1\/\$2/);
assert.match(commonSsrRoutes, /location \^~ \/stories\/\s*\{[\s\S]*?proxy_pass http:\/\/api_backend\/ssr\/stories\//);
