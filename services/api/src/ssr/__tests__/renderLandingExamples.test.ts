...
assert.match(enWithStoryHtml, /href="https:\/\/app\.wondertales\.com\/stories" class="cta-purple"/);

const emptyUkHtml = renderLandingHtml({ locale: 'uk', exampleStories: [] });

assert.match(emptyUkHtml, /Історій цією мовою поки що немає/);
assert.match(emptyUkHtml, /href="https:\/\/app\.wondertales\.com\/uk\/wizard" class="cta-purple"/);
