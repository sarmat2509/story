-- Migration 0008: Add Voice Catalog and Age Groups Reference System
-- This migration adds:
...
CREATE UNIQUE INDEX age_groups_slug_idx ON age_groups(slug);
CREATE INDEX age_groups_sort_order_idx ON age_groups(sort_order);

-- Insert initial age groups
INSERT INTO age_groups (slug, name_key, min_months, max_months, sort_order) VALUES
  ('1y', 'age_groups.1y.name', 12, 24, 1),
  ('2-3', 'age_groups.2_3.name', 24, 48, 2),
  ('4-5', 'age_groups.4_5.name', 48, 72, 3),
  ('6-8', 'age_groups.6_8.name', 72, 108, 4),
  ('9-12', 'age_groups.9_12.name', 108, NULL, 5);
...
CREATE INDEX voice_age_groups_voice_id_idx ON voice_age_groups(voice_id);
CREATE INDEX voice_age_groups_age_group_id_idx ON voice_age_groups(age_group_id);
