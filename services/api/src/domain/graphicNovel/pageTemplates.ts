...
const PANEL_GUTTER_PX = Math.round((TEMPLATE_WIDTH * 0.04) / 2);
...
function rowLayout(rowWeights: number[], columnWeightsByRow: number[][]): Rect[] {
  return splitRows(PAGE_FRAME_PX, rowWeights).flatMap((row, index) =>
    splitColumns(row, columnWeightsByRow[index] ?? [1])
